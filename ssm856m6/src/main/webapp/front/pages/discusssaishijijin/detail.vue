...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusssaishijijinCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusssaishijijinCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusssaishijijin');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','discusssaishijijin');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('discusssaishijijin', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusssaishijijin', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscusssaishijijin/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discusssaishijijin', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discusssaishijijin', this.detail);
				this.$utils.msg('审核成功');
