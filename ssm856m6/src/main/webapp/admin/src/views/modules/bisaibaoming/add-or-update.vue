<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="球队选择" prop="qiuduixuanze">
          <el-select  @change="qiuduixuanzeChange" v-model="ruleForm.qiuduixuanze" placeholder="请选择球队选择">
            <el-option
                v-for="(item,index) in qiuduixuanzeOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.qiuduixuanze" label="球队选择" prop="qiuduixuanze">
              <el-input v-model="ruleForm.qiuduixuanze" 
                placeholder="球队选择" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.beizhu = true;
          ...
          if(o=='qiuduixuanze'){
            this.ruleForm.qiuduixuanze = obj[o];
	    this.ro.qiuduixuanze = true;
            continue;
          }
      ...
      this.$http({
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/qiuduixinxi/qiuduimingcheng`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.qiuduixuanzeOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    qiuduixuanzeChange () {
      this.$http({
        url: `follow/qiuduixinxi/qiuduimingcheng?columnValue=`+ this.ruleForm.qiuduixuanze,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.duizhangzhanghao){
                this.ruleForm.duizhangzhanghao = data.data.duizhangzhanghao
              }
